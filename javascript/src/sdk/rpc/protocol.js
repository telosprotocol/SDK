...
class Protocol {
    ...
    encode(params, body) {
        ...
        params["body"] = body_params_wrap.str;
        for (const key in params) {
            if (this.is_param_key_effect(key)) {
                body.str += (0 === body.length ? "" : "&");
                body.str += encodeURI(key);
                body.str += "=";
                body.str += params[key];
            }
        }
    ...
    encode_transaction_params(root, tx) {
        ...
        s_action_json['action_param'] = "0x" + StringUtil.bytes2hex(source_action.get_action_param());
        ...
        t_action_json['action_param'] =  "0x" + StringUtil.bytes2hex(target_action_param);
    ...
    encode_body_params(params, root) {
        if(undefined === this.m_param_names) {
            return true;
        }
        if(!root.hasOwnProperty('params')) {
            root['params'] = {};
        }
        for (let name of this.m_param_names) {
...
class AccountInfoCmd  extends Protocol {
    ...
    decode(params,task_info) {
        ...
        if(undefined !== data) {
            ...
            result.set_last_xxhash64(data['last_hash_xxhash64']);
            result.set_last_unit_height(Number(data['last_unit_height']));
        }
...
class AccountTransactionCmd extends Protocol {
    ...
    decode(params, task_info) {
        ...
        result.set_last_trans_hash(data['last_trans_hash']);
...
class TransferCmd extends Protocol {
    ...
    encode_body_params(params,root) {
        ...
        for (let name of this.m_param_names) {
            const val_find = params[name];
            if (undefined !== val_find) {
                if(name === "amount" || name === "nonce") {
                    root['params'][name] = Number(val_find);
                } else {
                    root['params'][name] = val_find;
                }
            } else {
...
class KeyStoreCmd extends Protocol {
    ...
    encode_body_params(params,root) {
        ...
        for (let name of this.m_param_names) {
            const val_find = params[name];
            if (undefined !== val_find) {
                if(name === "amount" || name === "nonce" || name === "type") {
                    root['params'][name] = Number(val_find);
                } else {
                    root['params'][name] = val_find;
                }
...
class GetBlockCmd extends Protocol {
    constructor() {
        super();
    }
    encode_body_params(params, root) {
