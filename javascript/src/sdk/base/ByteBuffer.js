...
var ByteBuffer = function (org_buf,offset) {
    ...
    **/
    this.string = function(val,index){
        if(val == undefined || val == null){
           var len = _org_buf['readInt32'+_endian+'E'](_offset);
           _offset+=4;
...
/*
    ...
    this.int64 = function (val) {
        if (val < -0x8000000000000000 || val > 0x7ffffffffffffc00) {
    ...
    this.uint64 = function (val) {
        if (val < 0 || val > 0xfffffffffffff800) {
            throw new Error("Value out of range.");
        }
        let lowWord = val % 0x100000000;
        let highWord = Math.floor(val / 0x100000000);
        this.uint32(lowWord);
        this.uint32(highWord);
        return this;
    };
